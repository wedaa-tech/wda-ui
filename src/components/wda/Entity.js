...
import { entityPreFlightTemplate, entityTuplesPreFlightTemplate } from './assert';
        ...
        <AccordionItem>
                    ...
                    <HStack>
                        <Box as="span" flex="1" textAlign="left">
                            <Editable defaultValue={'Entity'}>
                                <EditablePreview />
                                <EditableInput
                                    onChange={({ target }) => handleInputChange('entityName', target.value)}
            ...
            <AccordionPanel pb={4}>
                {Object.keys(entityTuples).map((entityTuples, id) => {
                    return <EntityInput id={id} key={id} entityTuples={entityTuples} setEntityTuples={setEntityTuples} />;
                })}
                <Button size="xs" leftIcon={<AddIcon />} onClick={addEntityTuples} marginRight="10px" marginTop="10px">
                    Add
                </Button>
        ...
        <Stack direction="row" h="55px">
            <Editable defaultValue="Enter Attribute" width="150px">
                ...
                <EditableInput
                    key="tupleName"
                    name="tupleName"
                    onChange={({ target }) => handleInputChange('tupleName', target.value)}
            ...
            <Divider orientation="vertical" />
            <Button size={'xs'} leftIcon={<DeleteIcon />} style={{ marginTop: '5px' }}>
                Delete
            </Button>
        </Stack>
    );
