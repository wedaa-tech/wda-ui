...
function Projects() {
    ...
    const verifyData = async (data, id) => {
        try {
            const response = await fetch(process.env.REACT_APP_API_BASE_URL + '/api/user/' + id, {
                method: 'get',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: initialized ? `Bearer ${keycloak?.token}` : undefined,
                                ...
                                <Tr key={index}>
                                    ...
                                    <Td>
                                        <Button
                                            colorScheme="teal"
                                            variant="link"
                                            onClick={e => handleClick(project, 'Architecture', project.project_id)}
                                            _hover={{
                                                transform: 'scale(1.1)',
                                    ...
                                    {project.metadata?.deployment ? (
                                        <Td>
                                            <Button
                                                ...
                                                variant="link"
                                                onClick={e =>
                                                    handleClick(project.metadata?.deployment, 'Infrastructure', project.projectName)
                                                }
                                                _hover={{
                                                    transform: 'scale(1.1)',
                                                    transition: 'transform 0.3s',
                                            ...
                                            >
                                                {project.metadata.deployment.cloudProvider === 'azure' ? (
                                                    <img width="40px" src={azure} alt="azure" />
                                                ) : project.metadata.deployment.cloudProvider === 'aws' ? (
                                    ...
                                    <Td>
                                        <button style={{ paddingLeft: '7px' }} onClick={() => verifyData(project, project.project_id)}>
                                            <svg
                                                stroke="currentColor"
                                                fill="none"
                                                strokeWidth="2"
                                    ...
                                    <Td>
                                        <button
                                            ...
                                            onClick={() => handleButtonClick(project.project_id, project.projectName)}
                                        >
